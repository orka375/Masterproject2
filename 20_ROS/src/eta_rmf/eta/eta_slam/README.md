...
This package provides a launch file that wraps `slam_toolbox`'s `online_async_launch.py`, forwarding eta-specific configuration. This ensures compatibility across ROS 2 distributions (Humble, Jazzy) by delegating lifecycle node management to `slam_toolbox` itself.
...
```sh
ros2 launch eta_slam slam_toolbox_online_async.launch.py
```
...
|---|---|---|
| `slam_params_file` | `eta_slam/config/slam_toolbox_online_async.yaml` | Full path to the ROS 2 parameters file for the `slam_toolbox` node |
| `use_sim_time` | `false` | Use simulation/Gazebo clock |

For example, to pass a custom parameters file:

```sh
ros2 launch eta_slam slam_toolbox_online_async.launch.py slam_params_file:=<my_path>
```

Or to use simulation time:

```sh
ros2 launch eta_slam slam_toolbox_online_async.launch.py use_sim_time:=true
```
...
Go to [`eta_navigation`](../eta_navigation/README.md) to learn how.
