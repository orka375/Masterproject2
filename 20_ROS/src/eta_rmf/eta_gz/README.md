...
2. Install `eta_gz` package
   ```
   sudo apt install ros-humble-eta-gz
   ```
...
2. Run slam toolbox
    ...
    Configuration can be forwarded to the `slam_toolbox_node`. By default, the configuration parameters are obtained from [eta's configuration file](https://github.com/Ekumen-OS/eta/blob/humble/eta_slam/config/slam_toolbox_online_async.yaml). In case a custom file is wanted to be passed, simply use the launch file argument for indicating the path to a new file.
