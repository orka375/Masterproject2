...
> [!IMPORTANT]
> The hardware has been updated. If you are looking for the old design please go to [f7af556d00a099cf18a9d1e17eb7e725fa97e1da](https://github.com/Ekumen-OS/eta/tree/f7af556d00a099cf18a9d1e17eb7e725fa97e1da)
...
<img src="docs/layout/eta_diagram_arduino.jpg" />
...
For uploading the microcontroller firmware please refer to [`eta_firmware`](../eta_firmware/README.md) package.
...
You can either follow these steps or **rely on community contribution (Recommended) for installing this via ansible playbooks**: See https://github.com/garyservin/eta_ansible_config

### Operative System

The recommended operative system depends on which ROS 2 distro you want to use:
- **kilted**: Ubuntu Mate 22.04 ARM64 - [Download](https://ubuntu-mate.org/download/arm64/)
...
Follow suit the instructions for installing next dependencies from binaries:
 - [ROS 2 kilted(If Ubuntu 22.04)](https://docs.ros.org/en/kilted/Installation/Ubuntu-Install-Debians.html)
...
To automatically source ROS installation, it is recommended to add `source /opt/ros/kilted/setup.bash` line to the `~/.bashrc` file.
...
rosdep update
```
Make sure to export the `ROS_DISTRO` environment variable (use `kilted` or `jazzy` depending on your installation):
```bash
export ROS_DISTRO=kilted  # or jazzy
```
And then proceed to install the workspace dependencies:
```bash
rosdep install --from-paths src -i -y -r
```
Note that option `-r` has been added. For ARM based processors, there are missing packages, e.g. those related to simulation. We would not try to run the simulation in the compute platform of eta, however for convenience it is added here.
...
See [ROS_DOMAIN_ID](https://docs.ros.org/en/kilted/Concepts/Intermediate/About-Domain-ID.html)
...
[`eta_bringup`](../eta_bringup/launch/teleop_joystick.launch.py) package provides a launch file for launching the corresponding `ROS 2` nodes for teleoperating the robot using a joystick.
