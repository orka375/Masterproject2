...
This package provides a launch file that wraps `slam_toolbox`'s `online_async_launch.py`, forwarding eta-specific configuration. This ensures compatibility across ROS 2 distributions (kilted, Jazzy) by delegating lifecycle node management to `slam_toolbox` itself.
