...
The hardware-software-ros interaction in the `eta` project is developed using [ROS 2 Control](https://control.ros.org/master/index.html).

This package:
 - Implements `eta`'s [hardware interface](https://control.ros.org/master/doc/ros2_control/hardware_interface/doc/writing_new_hardware_interface.html).
 - Provides a communication with microcontroller:
   - `eta_base::MotorDriver` class is in charge of the Serial communication for commanding the motors.
...
The hardware interface accepts some parameters that are passed via the urdf description within the `ros2_control` tag (Check [`eta_description`](../eta_description/urdf/include/eta_control.urdf.xacro))
